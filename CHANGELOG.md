...
## v1.0.3-5 (2026-08-09) — 豆包回复语义抓取修复

### 🐛 修复
- 修复豆包网页更新/哈希类名变化后，回复已显示但无法被抓取并返回给 OpenAI 兼容客户端（如 Haven）的情况。
- 语义抓取器新增“发送后 DOM 变更”候选：只考虑发送后新增/变化、位于本次用户消息之后的内容，不回退到任意页面文本。
- 对无明确 assistant role/selector 的动态候选增加更长稳定等待，并排除侧栏、菜单、Toast/通知、输入框和提示回显。
- 新增动态 Shadow DOM 观察与 `QitongCapture` logcat 诊断，方便真实设备定位站点 DOM 再次变化。

### ✅ 回归
- API 35 WebView 端到端测试将豆包回复改为完全未知的哈希类名、无 role/data-testid，仍可抓取 `D:hello` 并继续转发得到 `S:Y:D:hello`。
>>>>>>> 9db1ccad067c8b9fe8c06715ef0b184bfa2f2268
